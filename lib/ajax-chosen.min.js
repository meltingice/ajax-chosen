...
this.options.data[this.options.jsonTermKey]=val;if(this.options.dataCallback!=null){this.options.data=this.options.dataCallback(options.data);}
_this=this;options=this.options;options.success=function(data){return _this.show_results(data);};return this.timer=setTimeout(function(){var chosenXhr;if(chosenXhr){chosenXhr.abort();}
return chosenXhr=$.ajax(options);},options.afterTypeDelay);};ajaxChosen.prototype.show_results=function(data){var items,nbItems,selected_values,_this;if(data==null){return;}
...
if($.inArray(value+"-"+text,selected_values)===-1){return $("<option />").attr('value',value).html(text).appendTo(_this.element);}}});if(nbItems){this.element.trigger("liszt:updated");}else{this.element.data().chosen.no_results_clear();this.element.data().chosen.no_results(this.search_field.val());}
