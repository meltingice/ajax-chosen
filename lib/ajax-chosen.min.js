...
selected_values=[];select.find('optgroup').each(function(){return $(this).remove();});select.find('option').each(function(){if(!$(this).is(":selected")){return $(this).remove();}else{return selected_values.push($(this).val()+"-"+$(this).text());}});items=callback(data,val);$.each(items,function(value,element){var group;if(element.group){group=$("<optgroup />").attr('label',element.text).appendTo(select);return $.each(element.items,function(value,text){if($.inArray(value+"-"+text,selected_values)===-1){return $("<option />").attr('value',value).html(text).appendTo(group);}});}else if($.inArray(value+"-"+element,selected_values)===-1){return $("<option />").attr('value',value).html(element).appendTo(select);}});select.trigger("liszt:updated");if(success!=null){success(data);}
