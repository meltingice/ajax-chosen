...
(function($) {
  ...
  return ajaxChosen = (function() {
    ...
    function ajaxChosen(element, settings, callback, chosenOptions) {
      ...
      this.search_field = this.element.next('.chzn-container').find(".search-field > input, .chzn-search > input");
    ...
    ajaxChosen.prototype.update_list = function(evt) {
      ...
      this.element.next('.chzn-container').find('.no-results').text(msg);
      ...
      if (this.options.dataCallback != null) {
        this.options.data = this.options.dataCallback(options.data);
      }
      _this = this;
