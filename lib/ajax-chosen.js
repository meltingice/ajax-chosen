...
(function($) {
  ...
  return $.fn.ajaxChosen = function(options, callback) {
    var afterTypeDelay, chosenXhr, jsonTermKey, minTermLength, select;
    select = this;
    chosenXhr = null;
    minTermLength = options.minTermLength || 3;
    afterTypeDelay = options.afterTypeDelay || 500;
    jsonTermKey = options.jsonTermKey || "term";
    this.chosen();
    this.next('.chzn-container').find(".search-field > input").bind('keyup', function() {
      ...
      if (val.length < minTermLength || val === $(this).data('prevVal')) {
        return false;
      }
